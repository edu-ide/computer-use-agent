...
* `type`: Type a string of text on the keyboard. IMPORTANT: Output the text EXACTLY as requested without any encoding, escaping, or modification. For Korean text, output the Hangul characters directly (e.g., "가방"). Do NOT output unicode escapes or random characters.
...
def get_computer_use_system_prompt(
    ...
    conversation = NousFnCallPrompt(
        ...
        messages=[
            Message(
                role="system",
                content=[ContentItem(text="You are a helpful assistant. You MUST use the available tools to answer the user request. When using a tool, you MUST output the JSON within <tool_call> tags.")],
            ),
        ],
